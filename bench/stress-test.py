...
def createFile(filename, ngroups, ntables, nrows, complevel, complib, recsize):
    ...
    for k in range(ngroups):
        ...
        for j in range(ntables):
            ...
            table = fileh.create_table(group, f'table{j:04d}', TestDict,
                                       f'Table{j:04d}',
...
def dump_refs(preheat=10, iter1=10, iter2=10, *testargs):
    ...
    track.update()
    print(f"Inc refs in function testMethod --> {rc2 - rc1:5d}", file=sys.stderr)
    ...
    print(f"Inc refs in function testMethod --> {rc3 - rc2:5d}", file=sys.stderr)
...
def testMethod(file, usearray, testwrite, testread, complib, complevel,
    ...
    if complevel > 0:
        print(f"Compression library: {complib}")
    if testwrite:
        t1 = time.time()
